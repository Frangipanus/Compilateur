...
let rec closure_exp (acomp: tdecl list ref)  clot (param:local_env) (env:local_env) (fcpur : int) (e : tbexpr) = 
  ...
  |EAsign(id, e) -> EAsign(Smap.find id env, fst (closure_exp acomp  clot param env fcpur e)), snd (closure_exp acomp  clot param env fcpur e) 
...
and closure_stmt acomp   clot param env fcpur (s : tstmt)  = match s.stmt with 
  |SBexpr(t) -> let e1, fcpur = closure_exp acomp  clot param env fcpur t in {stmt = SBexpr(e1); loc = s.loc; typ = s.typ}, fcpur, env
  |SDecl(id, t) -> Printf.printf "id = %d\n" fcpur;let e1, fcpur2 = closure_exp acomp  clot param (Smap.add id (fcpur) env ) (fcpur + 8 ) t in 
                    {stmt = SDecl(fcpur, e1); loc = s.loc; typ = s.typ}, fcpur2, (Smap.add id (fcpur) env )
  |SVar(id, t) ->Printf.printf "id = %d\n" fcpur; let e1, fcpur2 = closure_exp acomp  clot param (Smap.add id (fcpur) env ) (fcpur + 8 ) t in 
...
and closure_funbody (acomp: tdecl list ref)   clot param env fcpur (f : tfunbody) = 
  ...
  {formal = f.formal; body = fst (closure_exp acomp  (list_to_smap (VSet.elements free)) (list_to_smap par_lst) Smap.empty 0 f.body); typ = f.typ; clot = VSet.elements free }, snd  (closure_exp acomp  (list_to_smap (VSet.elements free)) (list_to_smap par_lst) Smap.empty 0 f.body)
...
let rec compile_expr  (e : pbexpr) = match e.bexpr with 
  ...
  |Eval(_,_) -> Printf.printf "wtf"; failwith "not yet"
  |Evar(v) -> (match  v with
