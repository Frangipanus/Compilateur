...
result:
  | ( LHOOK ; idl = separated_list(COMMA, IDENT) ; RHOOK )? ; t = type
    { (idl, t) }
;
...
type:
  ...
  | LPAR ; tl = separated_list(COOMMA, type) ; ARROW ; res = result {TMulFun(tl, res)}
;
   
atype : 
| s = IDENT; (LPAR ;LHOOK; ty = TYPE; RHOOK ; RPAR)? {AVar(s, ty)}
| LPAR ty = type RPAR {AType(ty)}
| LPAR RPAR {AEmpty}
;
